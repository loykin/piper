...
func (l *Loader) BindFlag(key string, flag *pflag.Flag) error {
	...
	l.loadMu.Lock()
	alreadyLoaded := l.cached != nil || l.loadErr != nil
	l.loadMu.Unlock()
	if alreadyLoaded {
		return fmt.Errorf("config: BindFlag(%q) called after Load; binding has no effect", key)
	}
	l.flags[key] = flag
...
func (l *Loader) explicitPrefix(prefix string) bool {
	...
	for _, key := range schemaKeys(reflect.TypeOf(RootConfig{}), "") {
		...
		if flag, ok := l.flags[key]; ok && flag.Changed {
			return true
		}
