...
// staleTime keeps a detail view opened right after its row was clicked in
// the list (see MLflowIntegrationsPage's onRowClick, which seeds this exact
// query key via setQueryData from the row's already-fetched data) from
// immediately re-fetching the same integration+backlog snapshot the list
// query just loaded.
export function useMLflowIntegration(id: string) { const projectId = useProjectId(); return useQuery({ queryKey: mlflowKeys.detail(projectId, id), queryFn: () => api.getIntegration(projectId, id), enabled: !!projectId && !!id, staleTime: 10_000 }) }
...
export function useMLflowRunLinks(runId: string) { const projectId = useProjectId(); return useQuery({ queryKey: mlflowKeys.run(projectId, runId), queryFn: () => api.listRunLinks(projectId, runId), enabled: !!projectId && !!runId, refetchInterval: query => query.state.data?.some(link => ['pending', 'syncing'].includes(link.sync_status)) ? 3000 : false, ...backgroundPollingNotifications }) }
