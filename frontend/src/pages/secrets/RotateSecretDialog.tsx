...
export default function RotateSecretDialog({
  target,
  rotateSecret,
  onClose,
}: {
  target: SecretMetadata | null
  rotateSecret: UseMutationResult<void, Error, { name: string; data: Record<string, string> }>
  onClose: () => void
}) {
  ...
  async function submit() {
    ...
    try {
      await rotateSecret.mutateAsync({ name: target.name, data })
  ...
  const canSubmit = entries.some(e => e.key.trim())
  ...
  return (
    <Dialog open={!!target} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <DialogFooter>
          <Button variant="outline" onClick={onClose} disabled={rotateSecret.isPending}>Cancel</Button>
          <Button onClick={() => void submit()} disabled={!canSubmit || rotateSecret.isPending}>
