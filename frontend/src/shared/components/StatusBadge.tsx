const colors: Record<string, string> = {
  ...
  // Notebook async lifecycle statuses
  provisioning: 'bg-violet-500/20 text-violet-300 border-violet-500/30',
  starting:     'bg-sky-500/20 text-sky-300 border-sky-500/30',
  stopping:     'bg-orange-500/20 text-orange-300 border-orange-500/30',
}

const PULSING_STATUSES = new Set(['running', 'provisioning', 'starting', 'stopping'])
...
export default function StatusBadge({ status }: { status: string }) {
  return (
    <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${colors[status] ?? colors.pending}`}>
      {PULSING_STATUSES.has(status) && <span className="mr-1 h-1.5 w-1.5 rounded-full bg-current animate-pulse" />}
      {status}
    </span>
  )
}
