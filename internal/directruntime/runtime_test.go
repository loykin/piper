...
func newTestRuntime(t *testing.T, driver pdriver.Driver, concurrency int) *Runtime {
	t.Helper()
	w, err := New(Config{
...
func TestNewRequiresConcurrencyAndResolveStorage(t *testing.T) {
	if _, err := New(Config{RuntimeID: "w", Driver: &fakeDriver{}}); err == nil {
		t.Fatal("expected error when Concurrency is unset")
	}
	if _, err := New(Config{RuntimeID: "w", Driver: &fakeDriver{}, Concurrency: 1}); err == nil {
...
func TestObserveRecoversAndReportsResult(t *testing.T) {
	...
	got := reported.Load()
	if got.TaskID != recovered.TaskID || got.RuntimeID != "test-worker" {
