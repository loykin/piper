...
func migrate(db *sql.DB, driver string) error {
	...
	default:
		return fmt.Errorf("no migrations defined for driver: %s", driver)
	}
	...
	for _, m := range migrations {
		...
		if _, err := tx.Exec(`INSERT INTO schema_migrations (version, desc) VALUES (?, ?)`, m.version, m.desc); err != nil {
			_ = tx.Rollback()
