...
func (r *projectMutationRepo) Complete(ctx context.Context, v *projectclient.Mutation) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		result, err := db.NamedExecContext(ctx, `UPDATE project_mutations SET response_status=:response_status,response_headers=:response_headers,response_body=:response_body,completed=TRUE WHERE project_id=:project_id AND idempotency_key=:idempotency_key AND request_hash=:request_hash`, v)
		if err != nil {
			return err
		}
