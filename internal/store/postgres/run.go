...
const pgRunSelectCols = `project_id, id, schedule_id, experiment, pipeline_name, status, started_at, ended_at, scheduled_at, pipeline_yaml, params_json, created_by, worker_id, worker_last_seen_at, cancel_requested_at`
...
func (r *runRepo) Get(ctx context.Context, projectID, id string) (*run.Run, error) {
	...
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		q := db.Rebind(`SELECT ` + pgRunSelectCols + ` FROM runs WHERE project_id=? AND id=?`)
		return db.GetContext(ctx, &v, q, projectID, id)
	})
...
func (r *runRepo) List(ctx context.Context, projectID string, filter run.RunFilter) ([]*run.Run, error) {
	...
	if metricSort {
		query = `SELECT r.project_id, r.id, r.schedule_id, r.experiment, r.pipeline_name, r.status, r.started_at, r.ended_at, r.scheduled_at, r.pipeline_yaml, r.params_json, r.created_by, r.worker_id, r.worker_last_seen_at, r.cancel_requested_at
FROM runs r
...
func (r *runRepo) ListTerminalBefore(ctx context.Context, projectID string, cutoff time.Time) ([]*run.Run, error) {
	var out []*run.Run
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		q := db.Rebind(`SELECT ` + pgRunSelectCols + ` FROM runs
			 WHERE project_id=? AND ended_at IS NOT NULL AND ended_at<?
			   AND status NOT IN ('running', 'scheduled')
			 ORDER BY ended_at ASC`)
		return db.SelectContext(ctx, &out, q, projectID, cutoff)
	})
	if out == nil {
		out = []*run.Run{}
	}
...
func (r *runRepo) Count(ctx context.Context, projectID string, filter run.RunFilter) (int, error) {
	...
	var count int
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		return db.GetContext(ctx, &count, db.Rebind(query), args...)
	})
	return count, err
}
...
func (r *runRepo) UpdateStatus(ctx context.Context, projectID, id, status string, endedAt *time.Time) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		q := db.Rebind(`UPDATE runs SET status=?, ended_at=? WHERE project_id=? AND id=?`)
		_, err := db.ExecContext(ctx, q, status, endedAt, projectID, id)
		return err
	})
}
...
func (r *runRepo) FinalizeStatusCAS(ctx context.Context, projectID, id, to string, endedAt *time.Time) (bool, error) {
	var affected int64
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		q := db.Rebind(`UPDATE runs SET status=?, ended_at=? WHERE project_id=? AND id=? AND status NOT IN ('success', 'failed', 'canceled')`)
		res, err := db.ExecContext(ctx, q, to, endedAt, projectID, id)
		if err != nil {
			return err
		}
...
func (r *runRepo) MarkRunning(ctx context.Context, projectID, id string, startedAt time.Time) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		q := db.Rebind(`UPDATE runs SET status='running', started_at=? WHERE project_id=? AND id=?`)
		_, err := db.ExecContext(ctx, q, startedAt, projectID, id)
		return err
	})
}
...
func (r *runRepo) SetWorkerID(ctx context.Context, projectID, id, workerID string) (bool, error) {
	var affected int64
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		q := db.Rebind(`UPDATE runs SET worker_id=? WHERE project_id=? AND id=? AND worker_id=''`)
		res, err := db.ExecContext(ctx, q, workerID, projectID, id)
		if err != nil {
			return err
		}
		affected, err = res.RowsAffected()
...
func (r *runRepo) TouchWorkerLastSeen(ctx context.Context, workerID string, runIDs []string) error {
	if len(runIDs) == 0 {
		return nil
	}
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		query, args, err := sqlx.In(
			`UPDATE runs SET worker_last_seen_at=? WHERE worker_id=? AND id IN (?)`,
			time.Now().UTC(), workerID, runIDs)
		if err != nil {
			return err
		}
...
func (r *runRepo) SetCancelRequested(ctx context.Context, projectID, id string) (bool, error) {
	var affected int64
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		q := db.Rebind(`UPDATE runs SET cancel_requested_at=? WHERE project_id=? AND id=? AND cancel_requested_at IS NULL`)
		res, err := db.ExecContext(ctx, q, time.Now().UTC(), projectID, id)
		if err != nil {
			return err
		}
		affected, err = res.RowsAffected()
...
func (r *runRepo) Delete(ctx context.Context, projectID, id string) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		q := db.Rebind(`DELETE FROM runs WHERE project_id=? AND id=?`)
