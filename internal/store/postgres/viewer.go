...
func (r *viewerRepo) List(ctx context.Context, projectID string) ([]*viewer.Viewer, error) {
	...
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		...
		return db.SelectContext(ctx, &vs, q, projectID)
	})
	if vs == nil {
		vs = []*viewer.Viewer{}
	}
...
func (r *viewerRepo) FindRunning(ctx context.Context, projectID, runID, stepName, artifact, typ string) (*viewer.Viewer, error) {
	...
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		q := db.Rebind(`
			...
			LIMIT 1`)
		return db.GetContext(ctx, &v, q, projectID, runID, stepName, artifact, typ)
	})
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
...
func (r *viewerRepo) UpdateStatus(ctx context.Context, id string, status viewer.Status, endpoint string, pid int, workDir string) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		q := db.Rebind(`UPDATE viewers SET status=?, endpoint=?, pid=?, work_dir=?, updated_at=? WHERE id=?`)
		_, err := db.ExecContext(ctx, q, string(status), endpoint, pid, workDir, time.Now().UTC(), id)
		return err
	})
}
...
func (r *viewerRepo) ListExpired(ctx context.Context) ([]*viewer.Viewer, error) {
	...
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		...
		return db.SelectContext(ctx, &vs, q, time.Now().UTC())
	})
	return vs, err
}
...
func (r *viewerRepo) MarkStaleFailed(ctx context.Context) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		q := db.Rebind(`UPDATE viewers SET status='failed', updated_at=? WHERE status IN ('starting','running')`)
...
func (r *viewerRepo) Delete(ctx context.Context, id string) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		q := db.Rebind(`DELETE FROM viewers WHERE id=?`)
