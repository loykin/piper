...
func toMeta(r *connectionRow) *connection.Metadata {
...
const connCols = `project_id, name, type, endpoint, disabled, last_used_at, last_tested_at, last_test_ok, last_test_message, created_at, updated_at`

func (r *connectionRepo) List(ctx context.Context, projectID string) ([]*connection.Metadata, error) {
	var rows []connectionRow
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		return db.SelectContext(ctx, &rows, `SELECT `+connCols+` FROM connections WHERE project_id=? ORDER BY name`, projectID)
	})
	if err != nil {
		return nil, err
	}
...
func (r *connectionRepo) Get(ctx context.Context, projectID, name string) (*connection.Metadata, error) {
	...
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		return db.GetContext(ctx, &row, `SELECT `+connCols+` FROM connections WHERE project_id=? AND name=?`, projectID, name)
	})
...
func (r *connectionRepo) Rotate(ctx context.Context, projectID, name string, encrypted []byte) error {
	...
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		...
		res, err := tx.ExecContext(ctx, `UPDATE connections SET updated_at=? WHERE project_id=? AND name=?`, now, projectID, name)
		if err != nil {
			return err
		}
		...
		if _, err := tx.ExecContext(ctx, `UPDATE connection_values SET active=FALSE WHERE project_id=? AND name=?`, projectID, name); err != nil {
			return err
		}
		var version int
		if err := tx.GetContext(ctx, &version, `SELECT COALESCE(MAX(version), 0) + 1 FROM connection_values WHERE project_id=? AND name=?`, projectID, name); err != nil {
			return err
		}
		if _, err := tx.ExecContext(ctx,
...
func (r *connectionRepo) Patch(ctx context.Context, projectID, name string, req connection.PatchRequest) error {
	...
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		if req.Enabled != nil {
			res, err := db.ExecContext(ctx, `UPDATE connections SET disabled=?, updated_at=? WHERE project_id=? AND name=?`, !*req.Enabled, now, projectID, name)
			if err != nil {
				return err
			}
			if n, _ := res.RowsAffected(); n == 0 {
		...
		if req.Endpoint != nil {
			res, err := db.ExecContext(ctx, `UPDATE connections SET endpoint=?, updated_at=? WHERE project_id=? AND name=?`, *req.Endpoint, now, projectID, name)
			if err != nil {
				return err
			}
			if n, _ := res.RowsAffected(); n == 0 {
...
func (r *connectionRepo) Delete(ctx context.Context, projectID, name string) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		res, err := db.ExecContext(ctx, `DELETE FROM connections WHERE project_id=? AND name=?`, projectID, name)
		if err != nil {
			return err
		}
		if n, _ := res.RowsAffected(); n == 0 {
...
func (r *connectionRepo) GetValue(ctx context.Context, projectID, name string) ([]byte, error) {
	...
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		return db.GetContext(ctx, &ciphertext,
			`SELECT ciphertext FROM connection_values WHERE project_id=? AND name=? AND active=TRUE ORDER BY version DESC LIMIT 1`,
			projectID, name)
	})
...
func (r *connectionRepo) MarkUsed(ctx context.Context, projectID, name string) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		_, err := db.ExecContext(ctx, `UPDATE connections SET last_used_at=? WHERE project_id=? AND name=?`, time.Now().UTC(), projectID, name)
		return err
	})
}
