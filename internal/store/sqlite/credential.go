...
const credentialCols = `project_id, name, kind, endpoint, keys_json, disabled, last_used_at, last_tested_at, last_test_ok, last_test_message, created_at, updated_at`

func (r *credentialRepo) List(ctx context.Context, projectID string, limit, offset int) ([]*credential.Metadata, error) {
	query := `SELECT ` + credentialCols + ` FROM credentials WHERE project_id=? ORDER BY name`
	args := []any{projectID}
	if limit > 0 {
		query += " LIMIT ? OFFSET ?"
	...
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		return db.SelectContext(ctx, &rows, query, args...)
	})
	if err != nil {
		return nil, err
	}
	...
	for i := range rows {
		out = append(out, credentialRecordMeta(&rows[i]))
	}
	return out, nil
}
...
func (r *credentialRepo) Count(ctx context.Context, projectID string) (int, error) {
	var count int
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		return db.GetContext(ctx, &count, `SELECT COUNT(*) FROM credentials WHERE project_id=?`, projectID)
	})
	return count, err
}
...
func (r *credentialRepo) Get(ctx context.Context, projectID, name string) (*credential.Metadata, error) {
	...
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		return db.GetContext(ctx, &row, `SELECT `+credentialCols+` FROM credentials WHERE project_id=? AND name=?`, projectID, name)
	})
	...
	return credentialRecordMeta(&row), nil
}
...
func (r *credentialRepo) Create(ctx context.Context, meta *credential.Metadata, encrypted []byte) error {
	...
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		...
		if _, err := tx.ExecContext(ctx,
			...
			if credentialSQLiteUniqueError(err) {
...
func (r *credentialRepo) Rotate(ctx context.Context, projectID, name string, encrypted []byte, keys []string) error {
	...
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		...
		defer func() { _ = tx.Rollback() }()
		res, err := tx.ExecContext(ctx, `UPDATE credentials SET keys_json=?, updated_at=? WHERE project_id=? AND name=?`, string(keysJSON), now, projectID, name)
		if err != nil {
			return err
		}
		...
		if _, err := tx.ExecContext(ctx, `UPDATE credential_values SET active=FALSE WHERE project_id=? AND name=?`, projectID, name); err != nil {
			return err
		}
		var version int
		if err := tx.GetContext(ctx, &version, `SELECT COALESCE(MAX(version), 0) + 1 FROM credential_values WHERE project_id=? AND name=?`, projectID, name); err != nil {
			return err
		}
		if _, err := tx.ExecContext(ctx,
...
func (r *credentialRepo) Patch(ctx context.Context, projectID, name string, req credential.PatchRequest) error {
	...
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		updated := false
		if req.Enabled != nil {
			res, err := db.ExecContext(ctx, `UPDATE credentials SET disabled=?, updated_at=? WHERE project_id=? AND name=?`, !*req.Enabled, now, projectID, name)
			if err != nil {
				return err
			}
			if n, _ := res.RowsAffected(); n == 0 {
		...
		if req.Endpoint != nil {
			res, err := db.ExecContext(ctx, `UPDATE credentials SET endpoint=?, updated_at=? WHERE project_id=? AND name=?`, *req.Endpoint, now, projectID, name)
			if err != nil {
				return err
			}
			if n, _ := res.RowsAffected(); n == 0 {
		...
		if !updated {
			var exists int
			if err := db.GetContext(ctx, &exists, `SELECT COUNT(*) FROM credentials WHERE project_id=? AND name=?`, projectID, name); err != nil {
				return err
			}
			if exists == 0 {
				return credential.ErrNotFound
...
func (r *credentialRepo) Delete(ctx context.Context, projectID, name string) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		res, err := db.ExecContext(ctx, `DELETE FROM credentials WHERE project_id=? AND name=?`, projectID, name)
		if err != nil {
			return err
		}
		if n, _ := res.RowsAffected(); n == 0 {
...
func (r *credentialRepo) GetValue(ctx context.Context, projectID, name string) ([]byte, error) {
	...
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		return db.GetContext(ctx, &ciphertext,
			`SELECT ciphertext FROM credential_values WHERE project_id=? AND name=? AND active=TRUE ORDER BY version DESC LIMIT 1`,
			projectID, name)
	})
...
func (r *credentialRepo) MarkUsed(ctx context.Context, projectID, name string) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		_, err := db.ExecContext(ctx, `UPDATE credentials SET last_used_at=? WHERE project_id=? AND name=?`, time.Now().UTC(), projectID, name)
		return err
	})
}
...
func credentialRecordMeta(row *credential.Record) *credential.Metadata {
	var keys []string
...
func credentialSQLiteUniqueError(err error) bool {
