...
func (r *federationRepo) SyncConfiguredMembers(ctx context.Context, homeID string, memberIDs []string, at time.Time) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		...
		if _, err := tx.ExecContext(ctx,
			`UPDATE federation_members SET enabled=0, status='offline', updated_at=? WHERE home_id=?`, at, homeID); err != nil {
			return err
		}
		for _, memberID := range memberIDs {
			if _, err := tx.ExecContext(ctx, `
				...
				VALUES (?, ?, 1, 'offline', ?, ?)
				ON CONFLICT(home_id, id) DO UPDATE SET enabled=1, status='offline', updated_at=excluded.updated_at`,
...
func (r *federationRepo) SetMemberConnected(ctx context.Context, homeID, memberID string, connected bool, at time.Time) error {
	...
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		...
		defer func() { _ = tx.Rollback() }()
		query := `UPDATE federation_members SET status=?, ` + timeColumn + `=?, updated_at=? WHERE home_id=? AND id=? AND enabled=1`
...
func (r *federationRepo) CreateProject(ctx context.Context, homeID string, value *project.Project, actorID string) error {
	...
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		...
		defer func() { _ = tx.Rollback() }()
		if err := requireConfiguredMemberSQLite(ctx, tx, homeID, value.OwnerMemberID); err != nil {
			return err
		}
		...
		if err := insertFederationAuditSQLite(ctx, tx, auditInput{
...
func (r *federationRepo) SetProjectOwner(ctx context.Context, homeID, projectID, memberID, actorID string, at time.Time) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		...
		defer func() { _ = tx.Rollback() }()
		if err := requireConfiguredMemberSQLite(ctx, tx, homeID, memberID); err != nil {
			return err
		}
		var current string
		if err := tx.GetContext(ctx, &current, `SELECT owner_member_id FROM projects WHERE id=?`, projectID); err != nil {
		...
		if _, err := tx.ExecContext(ctx, `UPDATE projects SET owner_member_id=?, updated_at=? WHERE id=?`, memberID, at, projectID); err != nil {
			return err
		}
		if err := insertFederationAuditSQLite(ctx, tx, auditInput{
...
func requireConfiguredMemberSQLite(ctx context.Context, tx *sqlx.Tx, homeID, memberID string) error {
	var count int
	if err := tx.GetContext(ctx, &count, `SELECT COUNT(*) FROM federation_members WHERE home_id=? AND id=? AND enabled=1`, homeID, memberID); err != nil {
		return err
	}
	if count != 1 {
		return federation.ErrMemberNotConfigured
...
func insertFederationAuditSQLite(ctx context.Context, tx *sqlx.Tx, value auditInput) error {
...
func (r *federationRepo) ListMembers(ctx context.Context, homeID string) ([]*federation.Member, error) {
	...
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		return db.SelectContext(ctx, &members, `
...
func (r *federationRepo) ListAuditEvents(ctx context.Context, homeID string, limit int) ([]*federation.AuditEvent, error) {
	...
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		return db.SelectContext(ctx, &events, `
