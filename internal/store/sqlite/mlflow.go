...
func (r *mlflowRepo) CountIntegrations(ctx context.Context, projectID string) (int, error) {
	var count int
	err := r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		return db.GetContext(ctx, &count, `SELECT COUNT(*) FROM mlflow_integrations WHERE project_id=?`, projectID)
	})
	return count, err
}
...
func (r *mlflowRepo) DeleteIntegration(ctx context.Context, projectID, id string) error {
	return r.Run(ctx, func(ctx context.Context, db *sqlx.DB) error {
		res, err := db.ExecContext(ctx, `DELETE FROM mlflow_integrations WHERE project_id=? AND id=?`, projectID, id)
		if err != nil {
			return err
		}
		if n, _ := res.RowsAffected(); n == 0 {
