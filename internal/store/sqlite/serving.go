...
const serviceSelectCols = `name, owner_id, run_id, artifact, status, endpoint, pid, yaml, created_at, updated_at`
...
func (r *servingRepo) Create(ctx context.Context, svc *serving.Service) error {
	...
	_, err := r.db.NamedExecContext(ctx,
		`INSERT INTO services (name, owner_id, run_id, artifact, status, endpoint, pid, yaml, created_at, updated_at)
		 VALUES (:name, :owner_id, :run_id, :artifact, :status, :endpoint, :pid, :yaml, :created_at, :updated_at)`,
		svc)
	return err
}
...
func (r *servingRepo) Update(ctx context.Context, svc *serving.Service) error {
	...
	_, err := r.db.ExecContext(ctx,
		`UPDATE services SET owner_id=?, run_id=?, artifact=?, status=?, endpoint=?, pid=?, yaml=?, updated_at=? WHERE name=?`,
		svc.OwnerID, svc.RunID, svc.Artifact, svc.Status, svc.Endpoint, svc.PID, svc.YAML, svc.UpdatedAt, svc.Name)
	return err
}
...
func (r *servingRepo) Upsert(ctx context.Context, svc *serving.Service) error {
	...
	_, err := r.db.ExecContext(ctx,
		...
		svc.Name, svc.OwnerID, svc.RunID, svc.Artifact, svc.Status, svc.Endpoint, svc.PID, svc.YAML, now, now)
	return err
}
