...
func (r *servingRepo) Delete(ctx context.Context, projectID, name string) error {
	...
	var svc serving.Service
	if err := r.db.GetContext(ctx, &svc, `SELECT `+serviceSelectCols+` FROM services WHERE project_id=? AND name=?`, projectID, name); err == nil {
	...
	_, err := r.db.ExecContext(ctx, `DELETE FROM services WHERE project_id=? AND name=?`, projectID, name)
	return err
}
