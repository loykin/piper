...
func (p *Piper) notebookPromotionRuntime(nb *notebook.NotebookServer) notebook.PromotionRuntime {
	if nb == nil {
		return notebook.PromotionRuntime{}
	}
	rt := notebook.PromotionRuntime{
		WorkerID: nb.WorkerID,
		Image:    nb.Image,
		Env:      nb.Env,
		WorkDir:  nb.WorkDir,
		Endpoint: nb.Endpoint,
	}
	if p.agentRegistry == nil || nb.WorkerID == "" {
		return rt
	}
	info, err := p.agentRegistry.Get(nb.WorkerID)
	if err != nil || info == nil {
		return rt
	}
	rt.Backend = info.Kind
	rt.Mode = info.Mode
	if rt.WorkerID == "" {
		rt.WorkerID = info.ID
	}
...
func (p *Piper) NotebookPromotionPreview(ctx context.Context, name string, target notebook.PromotionTarget) (*notebook.PromotionPreview, error) {
	nb, err := p.repos.Notebook.Get(ctx, name)
	if err != nil {
		return nil, err
	}
	if nb == nil {
	...
	validation := notebook.ValidatePromotion(nb, target, p.store != nil)
	return notebook.BuildPromotionPreview(nb, target, validation, p.notebookPromotionRuntime(nb)), nil
}

func (p *Piper) NotebookPromotionValidate(ctx context.Context, name string, target notebook.PromotionTarget) (*notebook.PromotionValidation, error) {
	nb, err := p.repos.Notebook.Get(ctx, name)
	if err != nil {
		return nil, err
	}
...
func (p *Piper) NotebookPromotionExport(ctx context.Context, name string, target notebook.PromotionTarget) (*notebook.PromotionExportResult, error) {
	preview, err := p.NotebookPromotionPreview(ctx, name, target)
	if err != nil {
		return nil, err
	}
	if preview.Validation.Status == "error" {
		return nil, fmt.Errorf("promotion validation failed: %s", strings.Join(preview.Validation.Messages, "; "))
	}

	bundleDir := filepath.Join(p.cfg.OutputDir, "promotions", name)
	if err := os.MkdirAll(bundleDir, 0755); err != nil {
		return nil, fmt.Errorf("create promotion dir: %w", err)
	}
	stamp := time.Now().UTC().Format("20060102T150405Z")
	bundleName := fmt.Sprintf("%s-%s.yaml", stamp, target)
	bundlePath := filepath.Join(bundleDir, bundleName)
	if err := os.WriteFile(bundlePath, []byte(preview.Draft), 0644); err != nil {
		return nil, fmt.Errorf("write promotion draft: %w", err)
	}
	exportedAt := time.Now().UTC()
	objectKey := ""
	if target == notebook.PromotionTargetObjectStore {
		if p.store == nil {
			return nil, fmt.Errorf("artifact store is not configured")
		}
		objectKey = filepath.ToSlash(filepath.Join("promotions", name, bundleName))
		file, err := os.Open(bundlePath)
		if err != nil {
			return nil, fmt.Errorf("open promotion bundle: %w", err)
		}
		stat, statErr := file.Stat()
		if statErr != nil {
			_ = file.Close()
			return nil, fmt.Errorf("stat promotion bundle: %w", statErr)
		}
		if err := p.store.Put(ctx, objectKey, file, stat.Size()); err != nil {
			_ = file.Close()
			return nil, fmt.Errorf("upload promotion bundle: %w", err)
		}
		_ = file.Close()
	}
	...
	if err != nil {
		return nil, fmt.Errorf("marshal promotion manifest: %w", err)
	}
	manifestPath := strings.TrimSuffix(bundlePath, ".yaml") + ".json"
	if err := os.WriteFile(manifestPath, manifestBytes, 0644); err != nil {
		return nil, fmt.Errorf("write promotion manifest: %w", err)
	}

	result := &notebook.PromotionExportResult{
	...
	if objectKey != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("open promotion manifest: %w", err)
		}
		manifestStat, statErr := manifestFile.Stat()
		if statErr != nil {
			_ = manifestFile.Close()
			return nil, fmt.Errorf("stat promotion manifest: %w", statErr)
		}
		manifestKey := strings.TrimSuffix(objectKey, ".yaml") + ".json"
		if err := p.store.Put(ctx, manifestKey, manifestFile, manifestStat.Size()); err != nil {
			_ = manifestFile.Close()
			return nil, fmt.Errorf("upload promotion manifest: %w", err)
		}
		_ = manifestFile.Close()
	}
	return result, nil
...
func (p *Piper) ListExports(_ context.Context, name string) ([]*notebook.PromotionExportRecord, error) {
	dir := filepath.Join(p.cfg.OutputDir, "promotions", name)
	entries, err := os.ReadDir(dir)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil
		}
		return nil, fmt.Errorf("list promotion exports: %w", err)
	}
	var out []*notebook.PromotionExportRecord
	for _, entry := range entries {
		...
		if err != nil {
			return nil, fmt.Errorf("read promotion manifest %q: %w", entry.Name(), err)
		}
		var rec notebook.PromotionExportRecord
		if err := json.Unmarshal(data, &rec); err != nil {
			return nil, fmt.Errorf("decode promotion manifest %q: %w", entry.Name(), err)
		}
		if rec.BundlePath == "" {
			rec.BundlePath = strings.TrimSuffix(path, ".json") + ".yaml"
		}
		if rec.ManifestPath == "" {
...
func (p *Piper) DownloadExport(_ context.Context, name, file string) (*notebook.PromotionDownload, error) {
	if strings.Contains(file, "..") || filepath.Base(file) != file {
		return nil, fmt.Errorf("invalid export file %q", file)
	}
	dir := filepath.Join(p.cfg.OutputDir, "promotions", name)
	...
	return &notebook.PromotionDownload{Name: file, ContentType: contentType, Reader: f}, nil
}

func (p *Piper) Preview(ctx context.Context, name string, target notebook.PromotionTarget) (*notebook.PromotionPreview, error) {
	return p.NotebookPromotionPreview(ctx, name, target)
}

func (p *Piper) Validate(ctx context.Context, name string, target notebook.PromotionTarget) (*notebook.PromotionValidation, error) {
	return p.NotebookPromotionValidate(ctx, name, target)
}

func (p *Piper) Export(ctx context.Context, name string, target notebook.PromotionTarget) (*notebook.PromotionExportResult, error) {
	return p.NotebookPromotionExport(ctx, name, target)
}
