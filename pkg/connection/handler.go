...
func (h *Handler) RegisterRoutes(rg *gin.RouterGroup) {
	rg.GET("/connections", h.list)
	rg.GET("/connections/:name", h.get)

	member := rg.Group("", project.RequireRole(security.ProjectRoleMember))
	member.POST("/connections", h.create)
	member.PATCH("/connections/:name", h.patch)
	member.POST("/connections/:name/rotate", h.rotate)
	member.POST("/connections/:name/test", h.test)
	member.DELETE("/connections/:name", h.delete)
}
...
func (h *Handler) test(c *gin.Context) {
	...
	name := strings.TrimSpace(c.Param("name"))
	var req TestRequest
	// Body is optional (registry needs no repo). Only reject non-empty malformed JSON.
