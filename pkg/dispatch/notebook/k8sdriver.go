...
// Individual notebook.NotebookServerSpec fields override these when non-empty.
...
func NewK8sDriver(k8s kubernetes.Interface, cfg K8sDriverConfig, repo notebook.Repository) *K8sDriver {
	ns := cfg.Namespace
	if ns == "" {
		ns = "default"
	}
...
func (d *K8sDriver) ProvisionVolume(ctx context.Context, vol *notebook.NotebookVolume, storageSize string) error {
...
// Returns a notebook.NotebookServer with Status=notebook.StatusStarting; SyncStatus will transition
// to notebook.StatusRunning once the pod is ready.
func (d *K8sDriver) Start(ctx context.Context, spec notebook.NotebookServerSpec, vol *notebook.NotebookVolume, _ string) (*notebook.NotebookServer, error) {
...
func (d *K8sDriver) Stop(ctx context.Context, nb *notebook.NotebookServer) error {
...
func (d *K8sDriver) DeprovisionVolume(ctx context.Context, vol *notebook.NotebookVolume) error {
	pvcName := d.pvcName(vol.ID)
...
func (d *K8sDriver) SyncStatus(ctx context.Context, servers []*notebook.NotebookServer) error {
	for _, nb := range servers {
		...
		if err != nil {
			if k8serrors.IsNotFound(err) {
				_ = d.repo.SetStatus(ctx, nb.Name, notebook.StatusFailed)
			}
			continue
		}
		...
		switch {
		case sts.Status.ReadyReplicas >= 1 && nb.Status != notebook.StatusRunning:
			_ = d.repo.SetStatus(ctx, nb.Name, notebook.StatusRunning)

		case sts.Status.ReadyReplicas == 0 && desired == 0 && nb.Status != notebook.StatusStopped:
