...
func newVol() *notebook.NotebookVolume {
	return &notebook.NotebookVolume{ID: "aaaabbbbccccdddd"}
}
...
func TestK8sDriver_Start_CreatesStatefulSetAndService(t *testing.T) {
	d, _ := newTestK8sDriver(defaultCfg())
	vol := &notebook.NotebookVolume{ID: "aaaabbbbccccdddd", WorkDir: "/home/jovyan"}
	...
	if nb.Status != notebook.StatusStarting {
...
func TestK8sDriver_Stop_NotFoundIsNoop(t *testing.T) {
	d, _ := newTestK8sDriver(defaultCfg())
	nb := &notebook.NotebookServer{Name: "ghost"}
	if err := d.Stop(context.Background(), nb); err != nil {
...
func TestK8sDriver_SyncStatus_StartingToRunning(t *testing.T) {
	...
	nb := &notebook.NotebookServer{Name: "my-nb", Status: notebook.StatusStarting}
	...
	if err := d.SyncStatus(context.Background(), []*notebook.NotebookServer{nb}); err != nil {
		t.Fatalf("SyncStatus: %v", err)
	}

	updated, _ := repo.Get(context.Background(), "my-nb")
	if updated.Status != notebook.StatusRunning {
...
func TestK8sDriver_SyncStatus_RunningToStopped(t *testing.T) {
	...
	nb := &notebook.NotebookServer{Name: "my-nb", Status: notebook.StatusRunning}
	_ = repo.Create(context.Background(), nb)
	...
	_ = d.SyncStatus(context.Background(), []*notebook.NotebookServer{nb})

	updated, _ := repo.Get(context.Background(), "my-nb")
	if updated.Status != notebook.StatusStopped {
...
func TestK8sDriver_SyncStatus_MissingStatefulSetSetsFailed(t *testing.T) {
	...
	nb := &notebook.NotebookServer{Name: "ghost", Status: notebook.StatusStarting}
	_ = repo.Create(context.Background(), nb)

	_ = d.SyncStatus(context.Background(), []*notebook.NotebookServer{nb})

	updated, _ := repo.Get(context.Background(), "ghost")
	if updated.Status != notebook.StatusFailed {
