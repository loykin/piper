...
func newNBRegistryWithWorker(addr string) *notebook.NotebookWorkerRegistry {
	r := notebook.NewNotebookWorkerRegistry()
	r.Register(&notebook.NotebookWorkerInfo{
		ID:       "nb-test-worker",
		Addr:     addr,
		LastSeen: time.Now(),
	})
...
func TestNBWorkerDriver_StartWorkerReturns200(t *testing.T) {
	...
	vol := &notebook.NotebookVolume{ID: "vol-ok", WorkDir: "/notebooks/nb-ok"}
	nb, err := driver.Start(context.Background(), spec, vol, "")
	if err != nil {
		t.Fatalf("Start() unexpected error: %v", err)
	}
	if nb == nil {
		t.Fatal("Start() returned nil notebook.NotebookServer")
	}
	if nb.Name != "nb-ok" {
		t.Errorf("nb.Name = %q, want %q", nb.Name, "nb-ok")
	}
	if nb.Status != notebook.StatusStarting {
		t.Errorf("nb.Status = %q, want %q", nb.Status, notebook.StatusStarting)
	}
}
...
func TestNBWorkerDriver_StartWorkerReturns202(t *testing.T) {
	...
	vol := &notebook.NotebookVolume{ID: "vol-202", WorkDir: wantWorkDir}
	nb, err := driver.Start(context.Background(), spec, vol, "")
	if err != nil {
		t.Fatalf("Start() unexpected error: %v", err)
	}
...
func TestNBWorkerDriver_StopNoWorker(t *testing.T) {
	...
	nb := &notebook.NotebookServer{Name: "nb-to-stop"}
...
func TestNBWorkerDriver_StopWorkerReturns204(t *testing.T) {
	...
	nb := &notebook.NotebookServer{Name: "nb-stop"}
...
func TestNBWorkerDriver_StopWorkerReturns404(t *testing.T) {
	...
	nb := &notebook.NotebookServer{Name: "nb-gone"}
	if err := driver.Stop(context.Background(), nb); err != nil {
