...
func registerServingHandlers(a *Agent) {
	_ = a.dispatcher.Register(iagent.MethodServingDeploy, func(ctx context.Context, payload json.RawMessage) (any, error) {
		var req servingDeployRequest
		if err := json.Unmarshal(payload, &req); err != nil {
			return nil, err
		}
		return a.deployServing(ctx, req)
	})
	_ = a.dispatcher.Register(iagent.MethodServingStop, func(ctx context.Context, payload json.RawMessage) (any, error) {
		var req servingStopRequest
		if err := json.Unmarshal(payload, &req); err != nil {
			return nil, err
		}
		return nil, a.stopServing(ctx, req)
	})
	_ = a.dispatcher.Register(iagent.MethodServingRestart, func(ctx context.Context, payload json.RawMessage) (any, error) {
		var req servingDeployRequest
		if err := json.Unmarshal(payload, &req); err != nil {
			return nil, err
		}
...
func (a *Agent) deployServing(ctx context.Context, req servingDeployRequest) (servingDeployResponse, error) {
	if req.S3URI == "" {
	...
	apiDeployments := a.cfg.K8sClient.AppsV1().Deployments(ns)
	...
	apiServices := a.cfg.K8sClient.CoreV1().Services(ns)
...
func (a *Agent) stopServing(ctx context.Context, req servingStopRequest) error {
	if req.Name == "" {
	...
	if err := a.cfg.K8sClient.AppsV1().Deployments(ns).Delete(ctx, name, metav1.DeleteOptions{}); err != nil && !k8serrors.IsNotFound(err) {
		return err
	}
	if err := a.cfg.K8sClient.CoreV1().Services(ns).Delete(ctx, name, metav1.DeleteOptions{}); err != nil && !k8serrors.IsNotFound(err) {
		return err
	}
	return nil
}

func (a *Agent) servingNamespace() string {
	if a.cfg.ServingNamespace != "" {
		return a.cfg.ServingNamespace
	}
