...
func (r *recordingAgentRPC) SendRPC(_ context.Context, agentID, method string, payload any, result any) error {
	...
	case iagent.MethodNotebookSyncStatus:
		...
		res.Statuses = map[string]string{"demo": StatusRunning}
	}
	return nil
}
...
func TestAgentDriverStartUsesVolumeAgent(t *testing.T) {
	...
	nb, err := driver.Start(context.Background(), spec, &NotebookVolume{ID: "vol-1", WorkerID: "agent-1", WorkDir: "/home/jovyan"}, "metadata:\n  name: demo\n")
	if err != nil {
...
func TestAgentDriverStopAndDeprovision(t *testing.T) {
	...
	if err := driver.Stop(context.Background(), &NotebookServer{Name: "demo", WorkerID: "agent-1"}); err != nil {
		t.Fatalf("Stop returned error: %v", err)
	}
	if err := driver.DeprovisionVolume(context.Background(), &NotebookVolume{ID: "vol-1", WorkerID: "agent-1"}); err != nil {
...
func TestAgentDriverSyncStatus(t *testing.T) {
	repo := newFakeRepo()
	if err := repo.Create(context.Background(), &NotebookServer{Name: "demo", WorkerID: "agent-1", Status: StatusStarting}); err != nil {
	...
	if err := driver.SyncStatus(context.Background(), []*NotebookServer{{Name: "demo", WorkerID: "agent-1"}}); err != nil {
