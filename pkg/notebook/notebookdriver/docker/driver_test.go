...
import (
	...
	"github.com/loykin/piper/pkg/notebook/notebookdriver/drivertest"
)

var _ notebookdriver.Driver = (*Driver)(nil)
var _ notebookdriver.Recoverable = (*Driver)(nil)

func TestDockerDriverContract(t *testing.T) {
	drivertest.RunContract(t, func() notebookdriver.Driver {
...
func TestDockerRuntimeContainerCreateOptions(t *testing.T) {
	...
	opts, err := rt.containerCreateOptions(notebookdriver.StartRequest{
...
func TestDockerRuntimeRejectsUnallowedVolume(t *testing.T) {
	...
	if _, err := rt.containerCreateOptions(notebookdriver.StartRequest{
		RuntimeName: "analysis",
		Name:        "analysis",
		Spec:        spec,
		WorkDir:     t.TempDir(),
...
func TestDockerGPUResources(t *testing.T) {
	allDS := &manifest.DriverDockerSpec{
		Deploy: &manifest.DockerDeploySpec{
			Resources: manifest.DockerDeployResources{
				Reservations: &manifest.DockerReservations{
					Devices: []manifest.DockerDevice{{
	...
	selectedDS := &manifest.DriverDockerSpec{
		Deploy: &manifest.DockerDeploySpec{
			Resources: manifest.DockerDeployResources{
				Reservations: &manifest.DockerReservations{
					Devices: []manifest.DockerDevice{{
...
func TestDockerRecoverRegistersBeforeWatching(t *testing.T) {
	...
	if err := rt.Recover(context.Background(), func(rec notebookdriver.RecoveredHandle) func(string) {
		registered = true
		if rec.ProjectID != "project-a" || rec.Name != "demo" || rec.RuntimeName != "project-a__demo" || rec.Port != 18888 {
			t.Fatalf("recovered = %#v", rec)
		}
		return func(string) {}
	}, func(notebookdriver.RecoveredHandle, string) {}); err != nil {
		t.Fatal(err)
	}
}
...
func TestDockerRecoverOnlyUsesCurrentWorkerContainers(t *testing.T) {
	cli := &recoveryDockerClient{
		items: []container.Summary{
			{ID: "other", State: container.StateRunning, Labels: map[string]string{
				dockerManagedLabel: "true", dockerNotebookLabel: "other", dockerRuntimeLabel: "project-b__other", dockerRuntimeIDLabel: "worker-2",
			}},
		},
	}
	rt, err := NewWithClient(Config{}, "worker-1", cli)
	...
	var exits []string
	if err := rt.Recover(context.Background(), func(notebookdriver.RecoveredHandle) func(string) {
		t.Fatal("another worker's container should not be recovered")
		return func(string) {}
	}, func(rec notebookdriver.RecoveredHandle, status string) {
...
func TestDockerRuntimeRequiresRuntimeID(t *testing.T) {
