...
func TestBaremetalDriverContract(t *testing.T) {
	drivertest.RunContract(t, func() pipelinedriver.Driver {
		d, err := New(Config{RuntimeID: "contract-test", MetaDir: t.TempDir()})
		if err != nil {
			t.Fatalf("New: %v", err)
		}
		return d
...
func TestBaremetalDriverStartWaitTerminalCompletion(t *testing.T) {
	...
	d, err := New(Config{RuntimeID: "worker-1", MetaDir: t.TempDir(), PiperBin: agentPath})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
...
func TestBaremetalDriverRecoverReattachesRunningProcess(t *testing.T) {
	metaDir := t.TempDir()
	d1, err := New(Config{RuntimeID: "worker-1", MetaDir: metaDir})
	if err != nil {
	...
	d2, err := New(Config{RuntimeID: "worker-1", MetaDir: metaDir})
	if err != nil {
