...
// It wraps pkg/k8s.Launcher and exposes the pipelinedriver.Driver interface,
...
import (
	...
	"github.com/loykin/piper/pkg/pipeline/pipelinedriver"
	k8slauncher "github.com/loykin/piper/pkg/pipeline/pipelinedriver/k8slauncher"
...
// It also implements pipelinedriver.Observable — the Worker must call
...
type Driver struct {
	...
	mu             sync.Mutex
	waiters        map[string]chan pipelinedriver.Exit // runtimeKey → signal channel
...
func New(cfg Config) (*Driver, error) {
	...
	d := &Driver{
		...
		waiters:        make(map[string]chan pipelinedriver.Exit),
...
func (d *Driver) Start(ctx context.Context, task *proto.Task, spec pipelinedriver.ExecSpec) (pipelinedriver.Handle, error) {
	if task == nil {
		return pipelinedriver.Handle{}, fmt.Errorf("k8s driver: task is required")
	}
	if spec.RuntimeKey == "" {
		return pipelinedriver.Handle{}, fmt.Errorf("k8s driver: runtime key is required")
	}
	// Image and namespace are pre-resolved by the K8s worker; drivers must not
	// re-derive them from the task payload.
	image := spec.Image
	if image == "" {
		return pipelinedriver.Handle{}, fmt.Errorf("k8s driver: spec.Image is required (resolve image before calling Start)")
	}
	namespace := spec.Namespace
	if namespace == "" {
		return pipelinedriver.Handle{}, fmt.Errorf("k8s driver: spec.Namespace is required")
	}
	...
	if err != nil {
		return pipelinedriver.Handle{}, fmt.Errorf("k8s driver: build agent args: %w", err)
	}
	launcher := d.launcher(namespace)
	runtimeKey, err := launcher.CreateJob(ctx, task, spec.RuntimeKey, image, agentArgs, nil)
	if err != nil {
		return pipelinedriver.Handle{}, fmt.Errorf("k8s dispatch: %w", err)
	}
	...
	ch := make(chan pipelinedriver.Exit, 1)
	d.mu.Lock()
...
func (d *Driver) Wait(ctx context.Context, handle pipelinedriver.Handle) (pipelinedriver.Exit, error) {
	...
	if ch == nil {
		return pipelinedriver.Exit{InfraFailure: fmt.Errorf("k8s job %q not tracked", handle.RuntimeKey)}, nil
	}
...
func (d *Driver) Stop(ctx context.Context, handle pipelinedriver.Handle, _ time.Duration) error {
	d.mu.Lock()
...
func (d *Driver) Recover(ctx context.Context) ([]pipelinedriver.Handle, error) {
	if len(d.cfg.Namespaces) == 0 {
		...
		if d.cfg.RuntimeID != "" {
			selector = k8smanifest.RuntimeSelector(d.cfg.RuntimeID)
		}
...
// Observe implements pipelinedriver.Observable. The Worker calls this in a
...
func (d *Driver) reconcileOnce(ctx context.Context) {
	for _, launcher := range d.snapshotLaunchers() {
		launcher.ReconcileJobs(ctx, func(_ context.Context, result proto.TaskResult) error {
			...
			if ch != nil {
				exit := pipelinedriver.Exit{Result: &result}
				select {
				case ch <- exit:
				default:
				}
...
func (d *Driver) forget(handle pipelinedriver.Handle) {
	d.mu.Lock()
...
var (
	_ pipelinedriver.Driver     = (*Driver)(nil)
	_ pipelinedriver.Observable = (*Driver)(nil)
	_ pipelinedriver.RunStopper = (*Driver)(nil)
)
