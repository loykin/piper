...
type Config struct {
	...
	// RuntimeID is the stable identity of this K8s runtime driver.
	// Jobs are labeled piper.io/runtime-id=RuntimeID so RecoverJobs only observes
	...
	for name, rec := range watched {
		...
		case jobSucceeded(job) || jobFailed(job):
			...
			if result, ok := l.readTerminationResult(ctx, job, rec); ok {
				result.RuntimeID = rec.RuntimeID
...
func (l *Launcher) jobLabels(task *proto.Task) map[string]string {
	...
	if l.cfg.RuntimeID != "" {
		labels[k8smanifest.LabelRuntimeID] = k8smanifest.LabelValue(l.cfg.RuntimeID)
	}
	return labels
}
