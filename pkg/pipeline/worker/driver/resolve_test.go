...
func TestResolveImagePriority(t *testing.T) {
	cases := []struct {
		...
		{name: "docker_step", runtime: "docker", stepImage: "step:1", pipelineImage: "pipeline:1", want: "step:1"},
		{name: "docker_pipeline", runtime: "docker", pipelineImage: "pipeline:1", want: "pipeline:1"},
		{name: "k8s_step", runtime: "k8s", stepImage: "step:1", pipelineImage: "pipeline:1", want: "step:1"},
		{name: "k8s_pipeline", runtime: "k8s", pipelineImage: "pipeline:1", want: "pipeline:1"},
		{name: "missing", runtime: "k8s", wantErr: true},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			task := makeImageTestTask(t, tc.runtime, tc.stepImage, tc.pipelineImage)
			got, err := ResolveImage(task, tc.runtime)
			if tc.wantErr {
				if err == nil {
...
func TestResolveImageDoesNotCrossRuntimeBoundaries(t *testing.T) {
	...
	if _, err := ResolveImage(task, "docker"); err == nil {
		t.Fatal("expected missing docker image error")
	}
}
...
func TestResolveNamespace(t *testing.T) {
	cases := []struct {
		...
		{name: "from_step", namespace: "gpu-jobs", want: "gpu-jobs"},
