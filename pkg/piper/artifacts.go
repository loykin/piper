...
func (h *apiHandler) handleArtifactList(w http.ResponseWriter, r *http.Request, runID string) {
	var (
		result []stepArtifacts
		err    error
	)
	if h.p.s3Cli != nil && h.p.cfg.S3.Bucket != "" {
		result, err = listArtifactsS3(r.Context(), h.p.s3Cli, h.p.cfg.S3.Bucket, runID)
	} else {
		result, err = listArtifactsLocal(h.p.cfg.OutputDir, runID)
	}
	if err != nil {
...
// rest = "artifact/subdir/file.ext"
func (h *apiHandler) handleArtifactDownload(w http.ResponseWriter, r *http.Request, runID, step, rest string) {
	...
	if h.p.s3Cli != nil && h.p.cfg.S3.Bucket != "" {
		...
		out, err := h.p.s3Cli.GetObject(r.Context(), &s3.GetObjectInput{
			Bucket: aws.String(h.p.cfg.S3.Bucket),
			Key:    aws.String(key),
		})
		if err != nil {
	...
	// Local
	localPath := filepath.Join(h.p.cfg.OutputDir, runID, step, filepath.FromSlash(rest))
	absPath, err := filepath.Abs(localPath)
	if err != nil {
		jsonErr(w, "invalid path", http.StatusBadRequest)
		return
	}
	baseAbs, _ := filepath.Abs(h.p.cfg.OutputDir)
...
func listArtifactsS3(ctx context.Context, client *s3.Client, bucket, runID string) ([]stepArtifacts, error) {
	...
	for _, s := range stepOrder {
		result = append(result, stepArtifacts{Step: s, Artifacts: stepMap[s]})
	}
	return result, nil
}
...
func newPiperS3Client(cfg S3Config) (*s3.Client, error) {
