...
func newServingAgentDriver(repo Repository) (*AgentDriver, *recordingServingAgentRPC) {
...
func TestServingAgentDriverStopAndRestart(t *testing.T) {
	repo := newStubServingRepo(&Service{Name: "demo", WorkerID: "agent-1"})
	driver, rpc := newServingAgentDriver(repo)
	spec := ModelService{}
	spec.Metadata.Name = "demo"
	spec.Spec.Model.FromURI = "s3://models/demo"

	if err := driver.Stop(context.Background(), &Service{Name: "demo", WorkerID: "agent-1"}); err != nil {
