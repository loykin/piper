...
func NewK8sDriver(k8s kubernetes.Interface, repo Repository) *K8sDriver {
	return &K8sDriver{k8s: k8s, repo: repo}
}

// Deploy creates a Kubernetes Deployment and Service for the given ModelService spec.
func (d *K8sDriver) Deploy(ctx context.Context, spec ModelService, art artifact.Resolved, yamlStr string) (*Service, error) {
	if art.S3URI == "" {
...
// Stop deletes the Kubernetes Deployment and Service for the given service record.
func (d *K8sDriver) Stop(ctx context.Context, svc *Service) error {
	ns := svc.K8sNamespace()
...
func (d *K8sDriver) Restart(ctx context.Context, spec ModelService, art artifact.Resolved, yamlStr string) error {
...
func (d *K8sDriver) deployK8s(ctx context.Context, svc ModelService, s3URI string, _ string) (*Service, error) {
	rt := svc.Spec.Runtime
	k := svc.Spec.K8s
	ns := k.Namespace
	if ns == "" {
