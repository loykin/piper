// Package docker implements the Docker-backed serving runtime.
...
func New(cfg Config) (*Driver, error) {
	if cfg.RuntimeID == "" {
		return nil, fmt.Errorf("docker serving runtime requires a stable runtime ID")
	}
	if cfg.Network == "" {
		cfg.Network = "bridge"
	}
...
func NewWithClient(cfg Config, cli dockerinfra.API) (*Driver, error) {
	...
	return &Driver{runtimeID: cfg.RuntimeID, cfg: cfg, client: cli, containers: make(map[string]*containerState)}, nil
}
...
// is still running. Health checks are owned by the caller.
func (d *Driver) Deploy(ctx context.Context, req servingdriver.DeployRequest) (endpoint string, err error) {
	image := req.Image
	if image == "" {
		if req.LogSink != nil {
			req.LogSink.Stop()
	...
	labels := map[string]string{
		...
		dockerRuntimeIDLabel: d.runtimeID,
	}
	...
	if req.ModelDir != "" {
		...
		mounts = append(mounts, mount.Mount{
			Type:     mount.TypeBind,
			Source:   modelDir,
			Target:   servingdriver.ContainerModelDir,
			ReadOnly: true,
		})
	}
...
// Recover scans for serving containers from a previous runtime instance and
// re-attaches exit watchers. Running containers call onRecovered; dead ones call onTerminal.
func (d *Driver) Recover(
	ctx context.Context,
	onRecovered func(servingdriver.RecoveredHandle) func(status string),
	...
	for _, item := range items {
		...
		if item.Labels[dockerRuntimeIDLabel] != d.runtimeID {
			continue
		}
		rec := servingdriver.RecoveredHandle{
...
func (d *Driver) listManagedContainers(ctx context.Context, runtimeName string) ([]container.Summary, error) {
	...
	filters = filters.Add("label", dockerRuntimeIDLabel+"="+d.runtimeID)
	if runtimeName != "" {
