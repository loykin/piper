...
import (
	...
	"github.com/loykin/piper/pkg/serving/servingdriver/drivertest"
)

func TestDockerDriverContract(t *testing.T) {
	drivertest.RunContract(t, func() servingdriver.Driver {
		d, err := NewWithClient(Config{RuntimeID: "contract-test"}, &recoveryClient{})
		if err != nil {
...
func TestRecoverReportsTerminalContainer(t *testing.T) {
	cli := &recoveryClient{items: []container.Summary{{
		...
		Labels: map[string]string{
			...
			dockerRuntimeLabel: "project-a__demo", dockerRuntimeIDLabel: "worker-1",
		},
	}}}
	d, err := NewWithClient(Config{RuntimeID: "worker-1"}, cli)
	if err != nil {
		t.Fatal(err)
	}
	var recovered bool
