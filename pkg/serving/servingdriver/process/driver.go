...
func New(cfg Config) *Driver {
	return &Driver{
		...
		pidDir:     filepath.Join(os.TempDir(), "piper-serving-pids", cfg.RuntimeID),
		collectors: make(map[string]func()),
	}
}

func (d *Driver) Deploy(_ context.Context, req servingdriver.DeployRequest) (string, error) {
	if req.RuntimeName == "" {
		d.stopSink(req)
...
func (d *Driver) Recover(_ context.Context, onRecovered func(servingdriver.RecoveredHandle) func(string), onTerminal func(servingdriver.RecoveredHandle, string)) error {
	...
	for _, entry := range entries {
		...
		handle := servingdriver.RecoveredHandle{ProjectID: meta.ProjectID, Name: meta.Name, RuntimeName: meta.RuntimeName, Port: meta.Port}
...
func (d *Driver) stopSink(req servingdriver.DeployRequest) {
	if req.LogSink != nil {
		req.LogSink.Stop()
	}
}
