...
func runStoreTests(t *testing.T, st storage.Store) {
	t.Helper()
...
func TestMemStore(t *testing.T) {
	runStoreTests(t, storage.NewMemStore())
}
...
func TestOpen_file_scheme(t *testing.T) {
	dir := t.TempDir()
	st, err := storage.Open("file://"+dir, "")
	if err != nil {
		t.Fatal(err)
	}
	if _, ok := st.(*storage.LocalStore); !ok {
		t.Errorf("expected *LocalStore, got %T", st)
	}
}

func TestOpen_unsupported_scheme(t *testing.T) {
	_, err := storage.Open("ftp://example.com/bucket", "")
	if err == nil {
...
func TestUploadPath_file(t *testing.T) {
	...
	if err := storage.UploadPath(ctx, ms, fpath, "run/step/out"); err != nil {
		t.Fatal(err)
	}
	keys := ms.Keys()
...
func TestUploadPath_directory(t *testing.T) {
	...
	if err := storage.UploadPath(ctx, ms, dir, "run/step/out"); err != nil {
		t.Fatal(err)
	}
	keys := ms.Keys()
	sort.Strings(keys)
...
func TestDownloadDir(t *testing.T) {
	...
	destDir := t.TempDir()
	if err := storage.DownloadDir(ctx, ms, "prefix/", destDir); err != nil {
		t.Fatal(err)
	}
...
func TestDownloadDir_empty_prefix_error(t *testing.T) {
	ctx := context.Background()
	ms := storage.NewMemStore()
	if err := storage.DownloadDir(ctx, ms, "noexist/", t.TempDir()); err == nil {
...
func TestServeHTTP(t *testing.T) {
	...
	if err := storage.ServeHTTP(ctx, ms, "my/key", rr); err != nil {
		t.Fatal(err)
	}
...
func TestServeHTTP_missing(t *testing.T) {
	ctx := context.Background()
	ms := storage.NewMemStore()
	err := storage.ServeHTTP(ctx, ms, "no/such", httptest.NewRecorder())
	if err != storage.ErrNotFound {
		t.Fatalf("expected ErrNotFound, got %v", err)
	}
}
