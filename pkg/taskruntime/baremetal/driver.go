...
func (d *Driver) Start(_ context.Context, task *proto.Task, spec taskruntime.ExecSpec) (taskruntime.Handle, error) {
	...
	if werr := d.writeMetadata(spec.RuntimeKey, handle, resultPath, pidFile); werr != nil {
		return taskruntime.Handle{}, fmt.Errorf("write metadata: %w", werr)
	}

	doneCh := make(chan taskruntime.Exit, 1)
	aj := &activeJob{
		handle:      handle,
		doneCh:      doneCh,
		resultPath:  resultPath,
	...
	if err != nil {
		...
		return taskruntime.Handle{}, fmt.Errorf("register process: %w", err)
	}
...
func (d *Driver) Wait(ctx context.Context, handle taskruntime.Handle) (taskruntime.Exit, error) {
	...
	if aj == nil {
		return taskruntime.Exit{InfraFailure: fmt.Errorf("job %q not tracked", handle.RuntimeKey)}, nil
	}
...
func (d *Driver) Stop(_ context.Context, handle taskruntime.Handle, grace time.Duration) error {
	if grace <= 0 {
...
func (d *Driver) Recover(_ context.Context) ([]taskruntime.Handle, error) {
...
func (d *Driver) onJobExit(runtimeKey, status string) {
	...
	exit := taskruntime.Exit{ResultPath: aj.resultPath}
...
func (d *Driver) writeMetadata(runtimeKey string, h taskruntime.Handle, resultPath, pidFile string) error {
