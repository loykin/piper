...
func (d *Driver) Start(ctx context.Context, task *proto.Task, spec taskruntime.ExecSpec) (taskruntime.Handle, error) {
	...
	containerResultFile := taskruntime.ContainerResultDir + "/" + spec.RuntimeKey + ".result.json"
	...
	agentArgs, err := taskruntime.BuildAgentExec(task, taskruntime.AgentExecConfig{
		...
		OutputDir:  taskruntime.ContainerOutputDir,
		InputDir:   taskruntime.ContainerInputDir,
		ResultFile: containerResultFile,
		ReportMode: taskruntime.ReportModeFile,
	})
	if err != nil {
		return taskruntime.Handle{}, fmt.Errorf("build agent args: %w", err)
	}
	...
	cmd := append([]string{taskruntime.ContainerPiperBin}, agentArgs...)
	...
	mounts := []mount.Mount{
		// piper binary (read-only)
		{
			Type:     mount.TypeBind,
			Source:   d.piperBin,
			Target:   taskruntime.ContainerPiperBin,
			ReadOnly: true,
		},
		// result directory
		{
			Type:   mount.TypeBind,
			Source: resultDir,
			Target: taskruntime.ContainerResultDir,
		},
		// output artifacts
		{
			Type:   mount.TypeBind,
	...
	if err != nil {
		return taskruntime.Handle{}, fmt.Errorf("container create: %w", err)
	}
	...
	if _, err := d.client.ContainerStart(ctx, resp.ID, dockerclient.ContainerStartOptions{}); err != nil {
		...
		return taskruntime.Handle{}, fmt.Errorf("container start: %w", err)
	}
...
func (d *Driver) Wait(ctx context.Context, handle taskruntime.Handle) (taskruntime.Exit, error) {
	...
	if containerID == "" {
		return taskruntime.Exit{InfraFailure: fmt.Errorf("container %q not tracked", handle.RuntimeKey)}, nil
	}
	...
	case err := <-waitResult.Error:
		return taskruntime.Exit{InfraFailure: fmt.Errorf("container wait: %w", err)}, nil
	case body := <-waitResult.Result:
		exit := taskruntime.Exit{ResultPath: handle.ResultPath}
...
func (d *Driver) Stop(ctx context.Context, handle taskruntime.Handle, grace time.Duration) error {
	d.mu.Lock()
	containerID := d.active[handle.RuntimeKey]
	delete(d.active, handle.RuntimeKey)
	d.mu.Unlock()
